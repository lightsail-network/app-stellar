...
- Authorization entries with `Address`, `Address V2` or `Address + Delegates` credentials are now shown during review, including the authorizing address, its nonce, signature expiration, and any delegated signers. Previously only source-account entries were displayed and the others were silently skipped.
- Delegate labels now include their parent authorization index, keeping delegated signer paths unambiguous when an operation contains multiple `Address + Delegates` entries.
